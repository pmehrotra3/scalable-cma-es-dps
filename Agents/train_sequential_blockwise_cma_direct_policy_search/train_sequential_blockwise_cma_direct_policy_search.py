# train_sequential_blockwise_cma_direct_policy_search.py
# Trains sequential_blockwise_cma_direct_policy_search (Custom implementation using pycma) on a Gymnasium environment and logs per-episode returns/lengths
# and per-episode system metrics (wall time, CPU time, RAM, CPU%) to CSV files.
#
# Usage: python train_sequential_blockwise_cma_direct_policy_search.py <env_name> [--total-timesteps N] [--num-runs N] [--block-size N]
#
...
from sequential_blockwise_cma_direct_policy_search import sequential_blockwise_cma_direct_policy_search
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train sequential_blockwise_cma_direct_policy_search with full episode and system logging.")
    ...
    # --- output directory ---
    out_dir = os.path.join("..", "..", "output", args.env_name, f"sequential_blockwise_cma_direct_policy_search [BLOCK_SIZE = {args.block_size}]")
    ...
    for run in range(1, args.num_runs + 1):
        ...
        model = sequential_blockwise_cma_direct_policy_search(env=env, block_size=args.block_size)
